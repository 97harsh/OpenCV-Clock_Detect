...
def store_raw_image():
    neg_image_link='http://www.image-net.org/api/text/imagenet.synset.geturls?wnid=n04105893'
...
def find_nonsense(): # removes images similar to images placed in nonsense folder
    for file_type in ['neg']:
        for img in os.listdir(file_type):
            for nonsense in os.listdir('Nonsense'):
                try:
                    image_path=str(file_type)+'/'+str(img)
                    # print(image_path)
                    useless=cv2.imread('Nonsense'+'/'+str(nonsense))
                    # print(useless.shape)
...
def pos_n_neg():
    for file_type in ['neg']:
        for img in os.listdir(file_type):
            if file_type=='neg':
                line='./'+file_type+'/'+img+'\n\n'
                with open('bg.txt','a') as f: #all files containing negative images
                    f.write(line)
